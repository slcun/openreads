...
curl -i http://127.0.0.1:8182/healthz
...
The default bind address is `0.0.0.0:8182`. Override with environment variables (see below).
...
| `BIND_PORT` | `8182` | Bind port |
| `DOUBAN_PROXY_CACHE_PATH` | `cache.db` | SQLite cache file path |
| `CACHE_TTL_SECONDS` | `86400` | Cache TTL (seconds, 24 h) |
| `MINIMUM_REQUEST_INTERVAL_SECONDS` | `5.0` | Minimum interval between upstream requests |
...
The app binds to `0.0.0.0:8182` by default and serves plain HTTP. If you need to access the proxy from a phone (e.g., to configure Openreads with a custom source URL), you must place an HTTPS reverse proxy in front — for example, Caddy or Nginx with a Let's Encrypt certificate.
...
- **Low-frequency, single-ISBN lookups only.** The proxy enforces a minimum 5-second interval between requests to Douban (with random 1.0~1.5x jitter). Bulk lookups will trigger rate limiting (`429`).
